...
There are several different types of accelerometers, and the most commonly used in inertial navigation systems are the \textit{closed loop pendulous accelerometers}, which can be implemented in \acrfull{mems}. \task{skriv om: }These work as a mass is maintained in its null position by an electromagnetic device, magnitude of the electric current in the coils change when there is a change of movement on the accelerometer the \cite{vik}.
